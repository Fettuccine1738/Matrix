...
public class RationalMatrix extends GenericMatrix<Rational>{
    ...
    public static Rational[][] upperTriangle(Rational[][] rational) {
        if (rational == null) throw new IllegalArgumentException("upper Triangle called with null argument");
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = i + 1; j < row; j++) {
                if (!(rational[i][i].equals(rezero())) && rational[j][i].equals(rezero())) {
                    // do nothing if already in row echecon form
                    continue;
                }
    ...
    public Rational[][] inverse(Rational[][] rational) {
        if (rational == null) throw new IllegalArgumentException("inverse called with null argument");
        validate(rational);
        int row = rational.length;
        ...
        for (int i = 0; i < row; i++) {
            Rational[] temp = rational[i]; // save row in case a swap is needed
        ...
        for (int i = row - 1; i > 0; i--) {
            for (int j = i - 1; j >= 0; j--) {
                if (!(rational[i][i].equals(rezero())) && rational[j][i].equals(rezero())) {
                    continue;
                } // no swaps needed, matrix already in row echelon form
                else {
    ...
    // helper method to find if matrix has no solution, unique / multi solution
    private static char solutionFlag(Rational[][] rational) {
        if (rational == null) throw new IllegalArgumentException("solution flag called with null argument");
    ...
    // return solution set from system of linear equations
    private static Rational[] backSubstitution(Rational[][] rational) {
        if (rational == null) throw new IllegalArgumentException("backSubstitution called with null argument");
    ...
    public static int rank(Rational[][] rational) {
        ...
        for (int i = 0; i < n; i++) {
            if(rational[i][m - 1].equals(rezero()) && rational[i][m].equals(rezero()) && rational[i][0].equals(rezero()))
                continue;
            rank++;
        }
        return rank;
